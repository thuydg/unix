...
#            print "mythread: ticking"
    ...
    def getattr(self, path):
        logging.debug(' duong getattr')
        return os.lstat("." + path)

    def readlink(self, path):
        logging.debug(' duong readlink')
        return os.readlink("." + path)

    def readdir(self, path, offset):
        logging.debug(' duong readdir')
        for e in os.listdir("." + path):
            yield fuse.Direntry(e)

    def unlink(self, path):
        logging.debug(' duong unlink')
        os.unlink("." + path)

    def rmdir(self, path):
        logging.debug(' duong rmdir')
        os.rmdir("." + path)

    def symlink(self, path, path1):
        logging.debug(' duong symlink')
        os.symlink(path, "." + path1)

    def rename(self, path, path1):
        logging.debug(' duong rename')
    ...
    def link(self, path, path1):
        logging.debug(' duong link')
        os.link("." + path, "." + path1)

    def chmod(self, path, mode):
        logging.debug(' duong chmod')
        os.chmod("." + path, mode)

    def chown(self, path, user, group):
        logging.debug(' duong chown')
